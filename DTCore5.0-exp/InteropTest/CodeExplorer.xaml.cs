...
{
    public partial class CodeExplorer
    {
        [DllImport("hid.dll")]
            ...
            var th = new Thread(() =>
                        {
                            ...
                            try
                            {
                                do
                                {
                                    this.Dispatcher.Invoke(() => { for (i = 0; i <= 255; i++) { mm.LongAtAbsolute(1L) = 0L; mm.ByteAt(0L) = (byte)i; if (HidD_GetFeature(h, mm, 65)) { s[i] = "HID CODE " + Conversion.Hex(i) + " = " + mm.IntAtAbsolute(1L); } x = 0; } });
                                    Thread.Sleep(1000);
        ...
        {
            if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(((MenuItem)sender).Header, "_Stop Watching Device", false)))
            {
                StopWatching();
