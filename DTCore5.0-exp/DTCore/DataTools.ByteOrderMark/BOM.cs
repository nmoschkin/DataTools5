...
namespace DataTools.ByteOrderMark
{
    ...
    public struct EscapeSequence
    {
        ...
        public override string ToString()
        {
            if (Control.ToString() != "None")
                ...
                return Conversions.ToString(Character);
        }
    }
    ...
    public struct SignalElement
    {
        ...
        public static SignalElement Parse(string subject)
        {
            var cc = new SignalElement();
            var s = TextTools.BatchParse(subject, "|");
            string[] sd;
            sd = TextTools.BatchParse(subject, Constants.vbTab);
            switch (sd.Length)
            {
                case 8:
                    {
                        ...
                        if (!string.IsNullOrEmpty(sd[6]))
                            cc.Escape = sd[6][1];
                        cc.Descrption = TextTools.SearchReplace(sd[7], @"\n", Constants.vbCrLf);
                        cc.Name = sd[5];
                        cc.Symbol = Conversions.ToChar(sd[4]);
                        break;
                    }
                ...
                case 6:
                    {
                        ...
                        cc.Code = (ControlCodes)Enum.Parse(typeof(ControlCodes), sd[3]);
                        cc.Acronym = cc.ToString();
                        cc.Descrption = TextTools.SearchReplace(sd[5], @"\n", Constants.vbCrLf);
                        cc.Name = sd[4];
                        break;
                    }
    ...
    public struct CodePageElement
    {
        ...
        public static CodePageElement Parse(string subject)
        {
            var cp = new CodePageElement();
            var s = TextTools.BatchParse(subject, "|");
            string[] sd;
            var cc = ControlCodes.NUL;
            cp.Type = (CPCharTypes)Enum.Parse(typeof(CPCharTypes), s[0]);
            if (!string.IsNullOrEmpty(Microsoft.VisualBasic.Strings.Trim(s[1])))
                cp.Unicode = (char)Conversion.Val(s[1]);
            if (s.Length == 3)
            {
                cp.Code = (uint)Conversion.Val(s[2]);
                return cp;
            }

            cp.Code = (uint)Conversion.Val(s[3]);
            if (cp.Type == CPCharTypes.Undefined)
                return cp;
            if (s[2].IndexOf(",") > -1)
            {
                sd = TextTools.BatchParse(s[2], ",");
                cp.Description = sd[0];
                cp.Signal = new SignalElement();
                if (Enum.TryParse(sd[1], out cc))
                {
                    cp.Signal = CPGlobal.Signals[cc];
                    cp.Ascii = Conversions.ToChar(cp.Signal.Code);
                }
                else if (sd[1].Length > 1 && sd[1][0] == '&')
                {
                    cp.Ascii = (char)Conversion.Val(sd[1]);
                }
                else
                {
                    cp.Ascii = sd[1][0];
            ...
            else if (s[2].Length > 1 && s[2][0] == '&')
            {
                cp.Ascii = (char)Conversion.Val(s[2]);
            }
            else
            {
                cp.Ascii = s[2][0];
    ...
    public struct BOM
    {
        ...
        static BOM()
        {
            if (mInit)
                return;
            mInit = true;
            UTF8 = new BOM(new[] { (byte)0xEF, (byte)0xBB, (byte)0xBF }, "UTF8");
            UTF16BE = new BOM(new[] { (byte)0xFE, (byte)0xFF }, "UTF16BE");

            // ' Windows default UTF16LEWindows")
            UTF16LE = new BOM(new[] { (byte)0xFF, (byte)0xFE }, "UTF16LE");
            UTF32BE = new BOM(new[] { (byte)0x0, (byte)0x0, (byte)0xFE, (byte)0xFF }, "UTF32BE");
            UTF32LE = new BOM(new[] { (byte)0xFF, (byte)0xFE, (byte)0x0, (byte)0x0 }, "UTF32LE");
            UTF7a = new BOM(new[] { (byte)0x2B, (byte)0x2F, (byte)0x76, (byte)0x38 }, "UTF7a");
            UTF7b = new BOM(new[] { (byte)0x2B, (byte)0x2F, (byte)0x76, (byte)0x39 }, "UTF7b");
            UTF7c = new BOM(new[] { (byte)0x2B, (byte)0x2F, (byte)0x76, (byte)0x2B }, "UTF7c");
            UTF7d = new BOM(new[] { (byte)0x2B, (byte)0x2F, (byte)0x76, (byte)0x2F }, "UTF7d");
            UTF7e = new BOM(new[] { (byte)0x2B, (byte)0x2F, (byte)0x76, (byte)0x38, (byte)0x2D }, "UTF7e");
            UTF1 = new BOM(new[] { (byte)0xF7, (byte)0x64, (byte)0x4C }, "UTF1");
            UTFEBCDIC = new BOM(new[] { (byte)0xDD, (byte)0x73, (byte)0x66, (byte)0x73 }, "UTFEBCDIC");
            SCSU = new BOM(new[] { (byte)0xE, (byte)0xFE, (byte)0xFF }, "SCSU");
            SCSU_BOCU1 = new BOM(new[] { (byte)0xFB, (byte)0xEE, (byte)0x28 }, "SCSU_BOCU1");
            SCSU_GB18030 = new BOM(new[] { (byte)0x84, (byte)0x31, (byte)0x95, (byte)0x33 }, "SCSU_GB18030");
        }

        public BOM(BOM bom)
        {
            _BOM = new byte[bom._BOM.Length];
        ...
        {
            int argresult = 0;
            return TextTools.CompareBytes(_BOM, (byte[])operand.BOM, result: ref argresult);
        }
        ...
        {
            var b = Parse(value);
            if (b._BOM is object && Conversions.ToBoolean(b._BOM.Length))
            {
                byte[] by;
                int c = value.Length;
                c -= b._BOM.Length;
            ...
            BOM._BOM = new byte[b._BOM.Length];
        ...
        {
            var b = new BOM();
            b._Type = (BOMTYPE)Conversions.ToShort(Enum.Parse(typeof(BOMTYPE), type, false));
                ...
                if (mt.MemberType == System.Reflection.MemberTypes.Field)
                {
                    if (mt.Name != "BOM" & mt.Name != "Type")
                    {
                        ...
                        if (y)
                        {
                            ...
                            {
                                b._Type = (BOMTYPE)Conversions.ToShort(Enum.Parse(typeof(BOMTYPE), fi.Name, false));
                                if (b._BOM is object && b._BOM.Length > 0)
                                    b._BOM = null;
                                b._BOM = new byte[by.Length];
        ...
        {
            var b = Parse(Encoding.ASCII.GetBytes(operand));
            return b;
        }
        ...
        {
            byte[] b;
            b = new byte[operand._BOM.Length];
        ...
        {
            Blob bl = operand._BOM;
            Blob.ReRank(ref bl, BlobTypes.Integer);
            return (MemPtr)bl;
        }
        ...
        {
            Blob bl = operand;
            Blob.ReRank(ref bl, BlobTypes.Byte);
            return new BOM((BOM)(byte[])bl);
        }
    ...
    [Serializable()]
    [TypeConverter(typeof(ExpandableObjectConverter))]
    public class ControlSignals : CollectionBase
    {
        public class SignalSorter : IComparer
        {
            public int Compare(object x, object y)
            {
                SignalElement s1 = (SignalElement)x;
                SignalElement s2 = (SignalElement)y;
                return (int)s1.Code - (int)s2.Code;
            }
        }

        public void Sort()
        {
            Array.Sort((Array)List, new SignalSorter());
        }

        public void AddSignal(string subject)
        {
            var cc = SignalElement.Parse(subject);
            List.Add(cc);
        }
        ...
        public SignalElement[] InnerArray
        {
            get
            {
                if (!CPGlobal.signalInit)
                {
                    CPGlobal.InitControlSignalCatalog();
                    CPGlobal.signalInit = true;
                }

                return (SignalElement[])InnerList.ToArray(typeof(SignalElement));
            }
        }
        ...
        public new SignalElement this[ControlCodes index]
        {
            get
            {
                if (!CPGlobal.signalInit)
                ...
                foreach (SignalElement cc in List)
                {
                    if (cc.Code == index)
                        return cc;
                }
        ...
        public new SignalElement this[int index]
        {
            get
            {
                ...
                return (SignalElement)List[index];
            }
        ...
        public int IndexOf(SignalElement value)
        {
            if (!CPGlobal.signalInit)
            {
                CPGlobal.InitControlSignalCatalog();
        ...
        public bool Contains(SignalElement value)
        {
        ...
        {
            if (!typeof(SignalElement).IsAssignableFrom(value.GetType()))
            {
    ...
    [Serializable()]
    [TypeConverter(typeof(ExpandableObjectConverter))]
    internal class CodePage : CollectionBase
    {
        private string _Name;
        private char[] mCA;

        internal class CPSorter : IComparer
        {
            public int Compare(object x, object y)
            {
                CodePageElement cp1 = (CodePageElement)x;
                CodePageElement cp2 = (CodePageElement)y;
                return (int)(cp1.Code - cp2.Code);
            }
        }
        ...
        public void Sort()
        {
            Array.Sort((Array)List, new CPSorter());
        }

        public void AddCode(string code)
        {
            var cp = CodePageElement.Parse(code);
            Add(cp);
            mCA = null;
        }
        ...
        public char[] GetCodeArray()
        {
            ...
            i = (int)objCP.Code;
            codes = new char[i];
            foreach (CodePageElement currentObjCP in List)
            {
                objCP = currentObjCP;
                codes[(int)objCP.Code] = objCP.Unicode;
            }
        ...
        public new CodePageElement this[int index]
        {
            get
            {
    ...
    [Serializable()]
    [TypeConverter(typeof(ExpandableObjectConverter))]
    internal class CodePageCollection : CollectionBase
    {
        ...
        [Browsable(true)]
        public new CodePage this[int index]
        {
            get
            {
        ...
        [Browsable(true)]
        public new CodePage this[string name]
        {
            get
            {
                name = name.ToLower();
        ...
        {
            if (CPGlobal.signalInit == false)
            {
                CPGlobal.InitControlSignalCatalog();
            }

            if (CPGlobal.cpInit == false)
            {
                CPGlobal.InitEBCDIC();
            }

            CPGlobal.cpInit = true;
        }
    }
    ...
    {
        internal static CodePageCollection CodesPages = new CodePageCollection();
        internal static ControlSignals Signals = new ControlSignals();
        internal static bool signalInit = false;
        internal static bool cpInit = false;
        ...
        public static string SafeTextRead(byte[] b)
        {
            var bm = new BOM();
            string s;
            var bOut = BOM.StripBOM(b, ref bm);
            switch (bm.Type)
            {
                case BOMTYPE.UTF16LE:
                    {
                ...
                case BOMTYPE.UTF32BE:
                    {
                        var nenc = new UTF32Encoding(true, false);
        ...
        public static byte[] SafeTextWrite(string subject, BOMTYPE enc = BOMTYPE.UTF16LE)
        {
            var bm = new BOM();
