...
namespace DataTools.Interop.Native
{
    ...
    public static class Utility
    {
        ...
        public static string GetEnumDescription(object value)
        {
            ...
            {
                if (Conversions.ToLong(fe.GetValue(value)) == Conversions.ToLong(value))
                {
        ...
        {
            return Conversions.ToBoolean(FileApi.MoveFile(oldPath, newPath));
        }
        ...
        public static bool DeleteWithExtremePrejudice(string directory, bool preserveRoot = true, bool warnConfirm = true)
        {
            ...
            if (warnConfirm)
            {
                if (Interaction.MsgBox("WARNING!  You are about to entirely destroy:" + Constants.vbCrLf + directory + "!" + Constants.vbCrLf + Constants.vbCrLf + "This action cannot be undone!  Proceed?", Constants.vbCritical | Constants.vbYesNo) == Constants.vbNo)
                    return false;
            }

            sh.hWnd = IntPtr.Zero;
            sh.pFrom = directory + Constants.vbNullChar;
        ...
        public static bool SetFileAttributes(string FileName, FileAttributes Attributes)
        {
            ...
            if (!FileExists(FileName))
                return false;
            SetFileAttributesRet = Conversions.ToBoolean(FileApi.pSetFileAttributes(FileName, (int)Attributes));
        ...
        public static bool PathExists(string Path)
        {
            ...
            else if (i == 3 & Strings.Mid(s, i - 1, 2) == @":\")
            {
                s = s + "*.*";
                df = true;
            }
        ...
        public static DateTime FileToLocal(PInvoke.FILETIME lpTime)
        {
            ...
            try
            {
                pDate = DateAndTime.DateSerial(lpSystem.wYear, lpSystem.wMonth, lpSystem.wDay);
