...
using DataTools.Interop.Native.Menu;
...
namespace DataTools.Interop.Desktop
{
    ...
    public sealed class SystemFileType : INotifyPropertyChanged, IDisposable
    {
        ...
        public bool Populate(IAssocHandler[] assoc = null, StandardIcons size = StandardIcons.Icon48)
        {
            if (assoc is null)
            ...
            try
            {
                ...
                RegistryKey pk2;
                if (pk is object && Conversions.ToString(pk.GetValue(null)) is object)
                {
                    pk2 = Registry.ClassesRoot.OpenSubKey(Conversions.ToString(pk.GetValue(null)));
                    if (pk2 is object)
                    {
                        string d = Conversions.ToString(pk2.GetValue(null));
                        if (string.Equals(d, _Desc) == false)
                        {
                            _Desc = Conversions.ToString(pk2.GetValue(null));
                            OnPropertyChanged("Description");
                        }

                        pk2.Close();
                        pk2 = Registry.ClassesRoot.OpenSubKey(Conversions.ToString(pk.GetValue(null)) + @"\DefaultIcon");
                        pk.Close();
                        if (pk2 is object)
                        {
                            d = Conversions.ToString(pk2.GetValue(null));
                            pk2.Close();
                            if (d is object)
                            {
                                int i = d.LastIndexOf(",");
    ...
    public sealed class AllSystemFileTypes : IDisposable, INotifyPropertyChanged
    {
        ...
        /// <remarks></remarks>
        public UIHandler HandlerFromAssocHandler(IAssocHandler assoc, string ext)
        {
    ...
    public static class Assoc
    {
        ...
        public static NativeMenu GetOpenWithMenu(string fileName, IntPtr openWithCmd, IntPtr hMenu)
        {
            ...
            foreach (IAssocHandler handler in assoc)
            {
                ...
                handler.GetIconLocation(out pth, out idx);
                int iix = (int)NativeShell.Shell_GetCachedImageIndex(pth, idx, 0U);
                icn = Resources.GetFileIconFromIndex(iix, (Resources.SystemIconSizes)Conversions.ToInteger(PInvoke.SHIL_SMALL));
