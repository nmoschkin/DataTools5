...
namespace DataTools.Interop.Native
{
    ...
    public static class DevPropDialog
    {
        ...
        public static void OpenDeviceProperties(string InstanceId, IntPtr hwnd = default)
        {
            ...
            DeviceProperties_RunDLL(hwnd, Process.GetCurrentProcess().Handle, argcmdLine, PInvoke.SW_SHOWNORMAL);
        }
    ...
    internal static class DevProp
    {
        ...
        public static extern IntPtr SetupDiGetClassDevs(Guid ClassGuid, IntPtr Enumerator, IntPtr hwndParent, ClassDevFlags Flags);
        ...
        public static extern IntPtr SetupDiGetClassDevsNoRef(Guid ClassGuid, IntPtr Enumerator, IntPtr hwndParent, ClassDevFlags Flags);

        [DllImport("setupapi.dll", CharSet = CharSet.Unicode, SetLastError = true, CallingConvention = CallingConvention.StdCall, PreserveSig = true)]
        public static extern bool SetupDiGetDevicePropertyKeys(IntPtr hDev, SP_DEVINFO_DATA DeviceInfoData, IntPtr PropertyKeyArray, uint PropertyKeyCount, ref uint RequiredPropertyKeyCount, uint Flags);
        ...
        public static extern bool SetupDiGetDeviceProperty(IntPtr DeviceInfoSet, SP_DEVINFO_DATA DeviceInfoData, DEVPROPKEY PropertyKey, out uint PropertyType, IntPtr PropertyBuffer, uint PropertyBufferSize, out uint RequiredSize, uint Flags);

        [DllImport("setupapi.dll", CharSet = CharSet.Unicode, SetLastError = true, CallingConvention = CallingConvention.StdCall, PreserveSig = true)]
        public static extern bool SetupDiLoadClassIcon(Guid ClassGuid, ref IntPtr hIcon, ref int MiniIconIndex);

        [DllImport("setupapi.dll", CharSet = CharSet.Unicode, SetLastError = true, CallingConvention = CallingConvention.StdCall, PreserveSig = true)]
        public static extern bool SetupDiLoadDeviceIcon(IntPtr hdev, SP_DEVINFO_DATA DeviceInfoData, uint cxIcon, uint cyIcon, uint Flags, ref IntPtr hIcon);

        [DllImport("setupapi.dll", EntryPoint = "SetupDiGetClassPropertyW", CharSet = CharSet.Unicode)]
        public static extern bool SetupDiGetClassProperty(Guid ClassGuid, DEVPROPKEY PropertyKey, ref int propertyType, IntPtr propertyBuffer, int propertyBufferSize, ref int RequiredSize, int Flags);

        [DllImport("setupapi.dll")]
        public static extern bool SetupDiDestroyDeviceInfoList(IntPtr DeviceInfoSet);

        [DllImport("setupapi.dll")]
        public static extern bool SetupDiEnumDeviceInterfaces(IntPtr DeviceInfoSet, SP_DEVINFO_DATA DeviceInfoData, Guid InterfaceClassGuid, uint MemberIndex, ref SP_DEVICE_INTERFACE_DATA DeviceInterfaceData);

        [DllImport("setupapi.dll")]
        public static extern bool SetupDiEnumDeviceInterfaces(IntPtr DeviceInfoSet, IntPtr DeviceInfoData, Guid InterfaceClassGuid, uint MemberIndex, ref SP_DEVICE_INTERFACE_DATA DeviceInterfaceData);

        [DllImport("setupapi.dll")]
        public static extern bool SetupDiEnumDeviceInterfaces(IntPtr DeviceInfoSet, IntPtr DeviceInfoData, Guid InterfaceClassGuid, uint MemberIndex, IntPtr DeviceInterfaceData);
        ...
        public static extern bool SetupDiGetDeviceInterfaceDetail(
            ...
            SP_DEVICE_INTERFACE_DATA beviceInterfaceData, 
        ...
        public struct DEVPROPKEY
        {
            ...
            {
                if (obj is DEVPROPKEY)
                {
                    if (((DEVPROPKEY)obj).fmtid.Equals(fmtid) && ((DEVPROPKEY)obj).pid == pid)
                    {
                        return true;
                    }
                }
        ...
        private static string GetKeyName(DEVPROPKEY dpk)
        {
            ...
            foreach (var fe in fi)
            {
                ...
                if (g == dpk)
                {
                    ...
                    string s = TextTools.SeparateCamel(strs[1]) + " Property: " + Constants.vbCrLf + TextTools.SeparateCamel(strs[2]);
                    return s;
                }
            }
        ...
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
        public struct DEVICE_CAPABALITIES
        {
            public ushort Size;
        ...
        public static int FlagsParse<T>(string values)
        {
            ...
            for (i = 0; i <= loopTo; i++)
            {
                ...
                if (enames.Contains(vs[i]))
                {
                    x = Conversions.ToInteger(Enum.Parse(typeof(T), vs[i]));
                    vOut = vOut | x;
                    e += 1;
                }
            }
